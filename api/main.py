...
pwd_ctx = None  # replaced by direct bcrypt calls
...
# ── AUTH HELPERS ──────────────────────────────────────────
# auth helpers defined above with direct bcrypt calls
...
def register(body: UserRegister, db=Depends(get_db)):
    ...
    return TokenOut(access_token=token, user_id=user_id, username=body.username, lang="en")
...
def login(form: OAuth2PasswordRequestForm = Depends(), db=Depends(get_db)):
    ...
    return TokenOut(access_token=token, user_id=row["id"], username=row["username"], lang=row["lang"])
...
def me(user=Depends(get_current_user)):
    return {"id": user["id"], "username": user["username"], "lang": user["lang"]}

# ── TRIP ROUTES ───────────────────────────────────────────
def row_to_trip(r): return dict(r)
...
def create_trip(body: TripIn, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    cur = db.execute("""INSERT INTO trips
        ...
        (user["id"], body.title, body.destination, body.start_date, body.end_date,
         body.budget, body.currency, body.notes, body.color, body.status, pos))
    db.commit()
...
def update_trip(trip_id: int, body: TripUpdate, user=Depends(get_current_user), db=Depends(get_db)):
    row = db.execute("SELECT id FROM trips WHERE id=? AND user_id=?", (trip_id, user["id"])).fetchone()
    if not row: raise HTTPException(404)
    db.execute("""UPDATE trips SET title=?,destination=?,start_date=?,end_date=?,
        budget=?,currency=?,notes=?,color=?,status=?,updated_at=datetime('now') WHERE id=?""",
        (body.title, body.destination, body.start_date, body.end_date,
         body.budget, body.currency, body.notes, body.color, body.status, trip_id))
    db.commit()
...
def reorder_trips(body: ReorderIn, user=Depends(get_current_user), db=Depends(get_db)):
    for pos, trip_id in enumerate(body.ids):
        db.execute("UPDATE trips SET position=? WHERE id=? AND user_id=?", (pos, trip_id, user["id"]))
    db.commit()
    return {"ok": True}

# ── DAY ROUTES ────────────────────────────────────────────
def check_trip(trip_id, user_id, db):
...
def create_day(trip_id: int, body: DayIn, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    cur = db.execute("""INSERT INTO days (trip_id,num,title,tagline,region,drive,dont_miss,position)
        VALUES (?,?,?,?,?,?,?,?)""",
...
def update_day(trip_id: int, day_id: int, body: DayUpdate, user=Depends(get_current_user), db=Depends(get_db)):
    check_trip(trip_id, user["id"], db)
    db.execute("""UPDATE days SET num=?,title=?,tagline=?,region=?,drive=?,dont_miss=?,position=?
        WHERE id=? AND trip_id=?""",
        (body.num, body.title, body.tagline, body.region, body.drive, body.dont_miss, body.position,
         day_id, trip_id))
    db.commit()
    return {"ok": True}
...
def reorder_days(trip_id: int, body: ReorderIn, user=Depends(get_current_user), db=Depends(get_db)):
    check_trip(trip_id, user["id"], db)
    for pos, day_id in enumerate(body.ids):
        db.execute("UPDATE days SET position=? WHERE id=? AND trip_id=?", (pos, day_id, trip_id))
    db.commit()
    return {"ok": True}
...
def create_attr(trip_id: int, day_id: int, body: AttrIn, user=Depends(get_current_user), db=Depends(get_db)):
    check_trip(trip_id, user["id"], db); check_day(day_id, trip_id, db)
    pos = db.execute("SELECT COALESCE(MAX(position),0)+1 FROM attractions WHERE day_id=?", (day_id,)).fetchone()[0]
    cur = db.execute("""INSERT INTO attractions (day_id,name,meta,cost,cost_type,maps_url,icon,position)
        VALUES (?,?,?,?,?,?,?,?)""",
...
def update_attr(trip_id: int, day_id: int, attr_id: int, body: AttrIn,
                user=Depends(get_current_user), db=Depends(get_db)):
    check_trip(trip_id, user["id"], db)
    db.execute("""UPDATE attractions SET name=?,meta=?,cost=?,cost_type=?,maps_url=?,icon=?,position=?
        WHERE id=? AND day_id=?""",
        (body.name, body.meta, body.cost, body.cost_type, body.maps_url, body.icon, body.position,
         attr_id, day_id))
    db.commit()
    return {"ok": True}
...
def update_note(trip_id: int, day_id: int, note_id: int, body: NoteIn,
                user=Depends(get_current_user), db=Depends(get_db)):
    check_trip(trip_id, user["id"], db)
    db.execute("UPDATE notes SET text=?,position=? WHERE id=? AND day_id=?",
        (body.text, body.position, note_id, day_id))
    db.commit()
    return {"ok": True}
...
def create_callout(trip_id: int, day_id: int, body: CalloutIn,
                   user=Depends(get_current_user), db=Depends(get_db)):
    check_trip(trip_id, user["id"], db); check_day(day_id, trip_id, db)
...
def delete_callout(trip_id: int, day_id: int, callout_id: int,
                   user=Depends(get_current_user), db=Depends(get_db)):
    check_trip(trip_id, user["id"], db)
